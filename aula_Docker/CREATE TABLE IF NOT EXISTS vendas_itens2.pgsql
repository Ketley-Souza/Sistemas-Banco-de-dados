...
SELECT
    produto_id, SUM(quantidade) AS total_vendidoqt, unidade
FROM 
    vendas_itens2
...
SELECT
    venda_id, SUM(valor_unitario*quantidade) AS total_vendas
FROM 
    vendas_itens2
...
SELECT
    venda_id, SUM(valor_unitario*quantidade) AS total_vendas,  data_venda
FROM 
    vendas_itens2
...
SELECT
    produto_id,
    MIN(valor_unitario) AS min_preco,
    MAX(valor_unitario) AS max_preco,
    ROUND(AVG(valor_unitario),2) AS avg_preco
FROM 
    vendas_itens2
GROUP BY
    produto_id
